...
The process starts with a grid or a defined space where each cell is in a superposition of all possible states (or patterns). As the algorithm progresses, it 'collapses' these possibilities one by one, reducing the uncertainty until a unique configuration is achieved that satisfies the predefined constraints. It is a fascinating mix of randomness and determinism, allowing for the creation of complex structures that appear both organic and highly structured.

A major limitation of the WFC algorithm is that, while it adheres to localized rules, the overall generation can sometimes lack coherence at the global scale. This can be seen in the following screenshot of an infinite city generator, where the broader structure seems disconnected despite local consistency.
...
Generates sudoku edges, tiles and states.
```python
# generate sudoku states / edges / tiles
...
Now that we have everything that we need the only remaining step is to run our WFC engine instance. This could be achieved this way.
```python
# create wfc instance and run it
sudoku_wfc = WFC(
      wfc_model = WFCModel(
