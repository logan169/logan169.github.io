...
We observe on previous screenshot that not all LLM models successfully generate working Houdini code, with some failing after reaching the iteration threshold.

By analyzing the work item logs, we can see that certain models struggled significantly more than others to adhere to the prompt requirements. Instead, their responses included extraneous elements such as:
...
To improve LLM performance in generating Houdini code, two strategies can be used:

- Fine-Tuning LLMs: This involves training pre-trained models on Houdini-specific data, such as documentation and example scripts, to help the model better understand Houdini's environment and generate more accurate code.

- Retrieval-Augmented Generation (RAG): RAG combines LLMs with real-time retrieval of relevant Houdini documentation and code, improving the model’s context and accuracy. By storing Houdini data in a vector database, RAG agents can provide the LLM with up-to-date, task-specific information, ensuring more reliable code generation.

Additionally, LangChain tools can be used to reduce randomness in model outputs by providing structured inputs and outputs, guiding the model’s responses. However, not all models support LangChain tools—models like DeepSeek and others may lack native integration, making it more challenging to enforce consistent output.

One potential improvement to the agent's logic is to test code line by line rather than as a whole block. This approach would reduce the amount of information the model needs to process at once, helping it generate more accurate and refined code with each iteration.

These strategies collectively enhance the reliability and effectiveness of LLMs in Houdini workflows.
